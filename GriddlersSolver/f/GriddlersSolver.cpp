...
#include "Estimators/FairEstimator.h"
...
void Alg(std::vector<itrec> &poprec)
{
	...
	FairEstimator estimator(griddler.GetColumnPattern(), griddler.GetColumnPatternSize());
	...
	while(!to_break)
	{
		...
		if(!to_break)
		{
			...
			/*
			for(int i = 0; i < pop; ++i) {
				std::cout <<  estimator( population_t[i] ) << " ";
			}*/
	...
	std::cout << ")\nFinal last population:" << std::endl;
